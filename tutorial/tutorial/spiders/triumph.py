...
class Triumph(CrawlSpider):
    ...
    def parse_item(self, response):
        ...
        item.add_value('price', self.price(raw_product))
        item.add_value('currency', self.currency(raw_product))
        item.add_value('gender', self.gender(raw_product))
        item.add_value('sub_category', self.sub_category(raw_product))
    ...
    def parse_sizes(self, response):
        ...
        self.image_urls(image_urls, image_urls_each_color)
        self.skus(skus, sizes, color, price, currency, image_urls_each_color)
        self.color_url_pop(color_urls)

        if len(color_urls) == 0:
            item = response.meta['item']
            item.add_value('image_urls', image_urls)
            item.add_value('skus', skus)
    ...
    def skus(self, skus, sizes, color, price, currency, image_urls_each_color):
        for i in range(0, len(sizes)):
            skus.append({'sku_id': color + '-' + self.clean(sizes[i]), 'color': color, 'size': self.clean(sizes[i]),
                         'price': price, 'currency': currency, 'image_urls': image_urls_each_color})

    def image_urls(self, image_urls, image_urls_each_color):
    ...
    def description(self, response):
        description = self.clean(response.css('.description ::text').extract_first())
        if description == '':
            return self.clean(response.css('.description p ::text').extract_first())
        return description
    ...
    def price(self, raw_product):
        return raw_product['productPrice']

    def currency(self, raw_product):
        return raw_product['productCurrency']

    def brand(self, raw_product):
        return raw_product['ecommerce']['detail']['products'][0]['brand']

    def sub_category(self, raw_product):
        return raw_product['ecommerce']['detail']['products'][0]['category']

    def retailer(self, response):
        return response.css('[id=logo-wrapper] a ::attr(title)').extract_first()
    ...
    def gender(self, raw_product):
        if len(re.findall(r'women', raw_product['ecommerce']['detail']['products'][0]['category'], re.I)):
            return 'female'
        if len(re.findall(r'men', raw_product['ecommerce']['detail']['products'][0]['category'], re.I)):
            return 'male'
        return 'female'
